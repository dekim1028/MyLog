...
const MyLog = ({newLog,myLog,selected,update,onChange,onAdd,onUpdate,onDelete,onChecked}) => {
    ...
    return (
        <MyLogBlock>
            <InputBlock>
                <div>
                    <InputBox type="text" name="name" placeholder="사이트 이름" value={newLog.name} onChange={onChange}/>
                    <InputBox type="text" name="url" placeholder="사이트 주소" value={newLog.url} onChange={onChange}/>
                </div>
                {
                    update?(
                        <AddBtn onClick={onUpdate}><RiEditLine/></AddBtn>
                    ):(
                        <AddBtn onClick={onAdd}><RiAddLine/></AddBtn>
                    )
                }
