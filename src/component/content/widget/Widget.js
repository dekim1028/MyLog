...
const Widget = ({ margin, info }) => {
	...
	return (
		<WidgetBlock style={{ marginLeft: margin ? '-50%' : '0' }}>
			{info.widget.weather.show && (
				<WeatherWidgetContainer thema={info.widgetThema} />
			)}
			{info.widget.calendar.show && (
				<CalenderWidgetContainer thema={info.widgetThema} />
			)}
			{info.widget.memo.show && (
				<MemoWidgetContainer thema={info.widgetThema} />
			)}
		</WidgetBlock>
	);
};
